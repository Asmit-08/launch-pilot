...
class DecisionService:
    ...
    def initialize_project(
        ...
        - No existing state:
            create initial beliefs, select the first constraint,
            create the first objective, and record the initial decision.

        The audit provides hypotheses.
        ...
        product = audit_result.get("product", {})
        ...
        #
        # For a beta/launch product, we still keep the payment
        # belief in the system, but we do not automatically give it
        # a weak score. The product/validation signal determines
        # whether it becomes the initial constraint.
        if project.get("stage") in ("beta", "launched"):
    ...
    def process_objective_completion(
        ...
        # =========================================================
        #
        # If another active objective already exists, this
        # completion has already been processed.
        #
        ...
        state_event_repository.create_event(
            ...
            payload={
                ...
                "old_confidence": belief.get("confidence", 0),
                "new_confidence": new_confidence,
                "evidence_count": evaluation["evidence_count"],
                "evidence_quality": evaluation["dominant_quality"],
                "reason": evaluation["reason"],
            },
        )

        # =========================================================
        # 8. FIND NEXT UNRESOLVED BELIEF
        # =========================================================
        ...
        # =========================================================
        # 9. NO UNRESOLVED BELIEFS REMAIN
        # =========================================================
        ...
        # =========================================================
        # 11. CREATE NEXT OBJECTIVE
        # =========================================================

        next_constraint_key = next_belief.get("type")
        ...
        next_objective = objective_repository.create_objective(
            project_id=project_id,
        ...
        # =========================================================
        # 14. RECORD NEXT OBJECTIVE EVENT
        # =========================================================
    ...
    def _evaluate_evidence(
        ...
        This version does not infer contradiction because the
        current evidence payload does not yet explicitly encode
        whether an observation supports or contradicts a belief.
        """
        ...
        return {
            ...
            "reason": (
                ...
                "available evidence remains anecdotal."
            ),
        }
    ...
    def _select_next_belief(
        ...
        The completed belief is excluded.
        """
    ...
    def _get_objective_config(
        ...
        if constraint_key in ("problem", "validation"):
        ...
        if constraint_key in ("solution_fit", "product"):
