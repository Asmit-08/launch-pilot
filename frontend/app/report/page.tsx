...
export default function DashboardPage() {
  ...
  /*
   * Convert any AI-generated value into something
   * that React can safely render.
   *
   * This protects the UI if the AI returns:
   *
   * - strings
   * - numbers
   * - booleans
  ...
  const toDisplayText = (value: any): string => {
    ...
    if (typeof value === "object") {
      ...
      // Generic object with a title
      if (typeof value.title === "string") {
        return value.title;
      }

      // Generic object with a strategy
      if (typeof value.strategy === "string") {
        return value.strategy;
      }

      // Generic object with a description
      if (typeof value.description === "string") {
        return value.description;
      }

      // Legacy nested risk object
      if (
        value.risk &&
      ...
      // Final fallback.
      // This guarantees React receives a string.
  ...
  const getRiskTitle = (risk: any): string => {
  ...
  const getMitigationText = (
    mitigation: any
  ): string => {
    return (
      toDisplayText(mitigation) ||
                    ...
                    (risk: any, index: number) => (
                      <li key={index}>
                        ...
                        {getRiskDescription(risk) && (
                          <p className="ml-4 mt-1 text-sm text-zinc-500">
                            {getRiskDescription(risk)}
                          </p>
                        )}
